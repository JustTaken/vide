...
fn global_listener(data: ?*anyopaque, registry: ?*c.wl_registry, name: u32, interface: [*c]const u8, _: u32) callconv(.C) void {
    const core: *Core = @ptrCast(@alignCast(data));
...
pub fn init() !Core {
    ...
    var core = Core{
        .display = c.wl_display_connect(null) orelse return error.Cagando,
        .registry = undefined,
        .seat = undefined,
    ...
    core.registry = c.wl_display_get_registry(core.display) orelse return error.Cagando;
    _ = c.wl_registry_add_listener(core.registry, &core.registry_listener, &core);
    _ = c.wl_display_roundtrip(core.display);

    core.surface = c.wl_compositor_create_surface(core.compositor) orelse return error.Cagando;
    _ = c.xdg_wm_base_add_listener(core.xdg_shell, &core.shell_listener, &core);

    core.xdg_surface = c.xdg_wm_base_get_xdg_surface(core.xdg_shell, core.surface) orelse return error.Cagando;
    _ = c.xdg_surface_add_listener(core.xdg_surface, &core.shell_surface_listener, &core);

    core.xdg_toplevel = c.xdg_surface_get_toplevel(core.xdg_surface) orelse return error.Cagando;
    ...
    // const vulkan = c.dlopen("libvulkan.so", 1) orelse return error.Cagando;
    //_ = c.dlclose(vulkan);

    return core;
}

pub fn deinit(core: *const Core) void {
