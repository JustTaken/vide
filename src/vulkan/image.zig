...
const Texture = struct {
    ...
    fn init(
        ...
        var handle: Texture = undefined;

        handle.sampler = sampler_init(device);
        handle.set = try graphics_pipeline.descriptors[1].get_set(device);
        handle.set.update(.{ .view = image.view, .sampler = handle.sampler }, null, 0, device);

        return handle;
    }

    fn sampler_init(device: *const Device) !c.VkSampler {
        ...
        try check(device.vkCreateSampler(device.handle, &info, null, &handle));
...
pub const Image = struct {
    ...
    fn init(
        ...
        try check(device.dispatch.vkCreateImage(device.handle, &info, null, &image.handle));

        var memory_requirements: c.VkMemoryRequirements = undefined;
        device.dispatch.vkGetImageMemoryRequirements(device.handle, image.handle, &memory_requirements);

        image.memory = device.allocate_memory(c.VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &memory_requirements);
        try check(device.dispatch.vkBindImageMemory(device.handle, image.handle, image.memory, 0));

        image.view = view_init(image.handle, format, device);
    ...
    fn copy_data(
        ...
        const buffer = Buffer.init(u8, device, c.VK_BUFFER_USAGE_TRANSFER_SRC_BIT, c.VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | c.VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, len);
        ...
        device.dispatch.vkdCopyBufferToImage(
            command_buffer,
            buffer.handle,
            self.handle,
        ...
        device.dispatch.vkdPipelineBarrier(
            command_buffer,
            c.VK_PIPELINE_STAGE_TRANSFER_BIT,
        ...
        try check(device.dispatch.vkUnmapMemory(device.handle, buffer.memory));
    ...
    fn deinit(self: *const Image, device: *const Device) void {
        device.dispatch.vkFreeMemory(device.handle, self.memory, null);
        device.dispatch.vkDestroyImage(device.handle, self.handle, null);
